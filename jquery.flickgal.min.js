...
false);c();var g=b(".nav",u),v=g.find("a[href^=#]"),H=v.parent(),L=!(!g.length||!v.length||!H.length);L&&(H.eq(0).addClass("selected"),v.bind(h,function(){var a=v.index(this);p(a);return false}).bind("click",function(){return false}));g=[A,h,z];if(d){var O=e[0];b.each(g,function(a,b){O.addEventListener(b,f,false)});O.addEventListener(B,K,false)}else e.bind(g.join(" "),f,false).bind(B,K,false);var w=b(".prev",u),x=b(".next",u),M=!(!w.length||!x.length);if(M){var N=function(){w.add(x).removeClass("off");
a===0?w.addClass("off"):a===k-1&&x.addClass("off")};w.bind(h,function(){a=a>0?a-1:m.infinitCarousel?k-1:a;p(a)});x.bind(h,function(){a=a<k-1?a+1:m.infinitCarousel?0:a;p(a)});N()}var F=0,J=0,I=0,t=0,E=false})}})(window.jQuery,this);
