...
u),v=g.find("a[href^=#]"),H=v.parent(),L=!(!g.length||!v.length||!H.length);L&&(H.eq(0).addClass("selected"),v.bind(h,function(){var a=v.index(this);p(a);return!1}).bind("click",function(){return!1}));g=[A,h,z];if(e){var O=d[0];b.each(g,function(a,b){O.addEventListener(b,f,!1)});O.addEventListener(B,K,!1)}else d.bind(g.join(" "),f,!1).bind(B,K,!1);var w=b(".prev",u),x=b(".next",u),M=!(!w.length||!x.length);if(M){var N=function(){w.add(x).removeClass("off");a===0?w.addClass("off"):a===k-1&&x.addClass("off")};
