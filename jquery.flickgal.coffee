...
browser = (if (userAgent.indexOf("webkit") >= 0) then BrowserType.WEBKIT else (if (userAgent.indexOf("gecko") >= 0) then BrowserType.GECKO else (if (userAgent.indexOf("msie") >= 0) then BrowserType.MSIE else (if (userAgent.indexOf("opera") >= 0) then BrowserType.OPERA else BrowserType.OTHER))))
isMobile = not not (userAgent.indexOf("iphone") >= 0 or userAgent.indexOf("ipad") >= 0 or userAgent.indexOf("android") >= 0)
...
TRANSLATE_PREFIX = (if (browser == BrowserType.WEBKIT) then "translate3d(" else "translate(")
TRANSLATE_SUFFIX = (if (browser == BrowserType.WEBKIT) then "px,0,0)" else "px,0)")
EventType = 
  ...
  TRANSITION_END: (if (browser == BrowserType.WEBKIT) then "webkitTransitionEnd" else (if (browser == BrowserType.OPERA) then "oTransitionEnd" else "transitionend"))
...
jQuery["fn"]["flickGal"] = (options) ->
  ...
  this["each"] ->
    ...
    getTranslateX = ->
      (if not (browser == BrowserType.GECKO) then $box["offset"]()["left"] else getGeckoTranslateX($box))
    ...
    useNav = not not ($nav["length"] and $navA["length"] and $navChildren["length"])
    $prev = $(".prev", $flickBox)
    $next = $(".next", $flickBox)
    useArrows = not not ($prev["length"] and $next["length"])
    if useArrows
      prevTappedHandler = ->
